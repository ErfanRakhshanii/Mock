...
mock.onAny().reply(config => {
    ...
    if (method === 'GET' && url === '/users') {
        const { family_name } = params;

        if (!family_name) {
            return [400, 'پارامتر family_name الزامی است.'];
        }

        const filteredUsers = users.filter(user => user.family_name === family_name);

        return [200, filteredUsers];
    }

    return [404, 'صفحه مورد نظر یافت نشد.'];
});

axiosInstance.get('/users', { params: { family_name: 'Dough' } })
    .then(response => {
        console.log('کاربران با نام خانوادگی Dough:', response.data);
    })
    .catch(error => {
